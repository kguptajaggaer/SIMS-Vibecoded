...
export default function SubkBatchApprovalPage() {
  const [user, setUser] = useState<User | null>(null)
  const [rows, setRows] = useState<ContractRow[]>([])
  const [loading, setLoading] = useState(true)
  const [processing, setProcessing] = useState(false)
  const [msg, setMsg] = useState<{ type: "success" | "error"; text: string } | null>(null)
  const [statusFilter, setStatusFilter] = useState("ready_for_portfolio_review")
  ...
  async function batchAction(action: "approve" | "reject") {
    ...
    const nextStatus =
      action === "approve"
        ? statusFilter === "ready_for_portfolio_review"
    ...
    if (error) {
      ...
      setMsg({
        type: "success",
        text: `${selected.length} contract${selected.length !== 1 ? "s" : ""} ${action === "approve" ? "approved" : "rejected"} successfully.`,
      })
      loadContracts()
    }
    setProcessing(false)
  ...
  const STATUS_OPTS = [
    { value: "ready_for_portfolio_review", label: "Ready for Portfolio Review" },
    { value: "ready_for_diversity_review", label: "Ready for Diversity Review" },
    ...
    <div>
      ...
      <div className="page-header" style={{ marginBottom: 20 }}>
        <div>
          ...
          <h1 className="page-title">SubK Contract Batch Approval</h1>
          <p className="page-subtitle">
            Select multiple contracts and approve or reject them in bulk.
          </p>
        </div>
      </div>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button
            ...
            style={{
              ...
              color: selected.length ? "white" : "#6b7280",
              border: "none",
              borderRadius: 5,
              fontSize: 13,
              fontWeight: 600,
      ...
      >
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
            <thead>
              <tr style={{ backgroundColor: "#f8fafc", borderBottom: "2px solid #e2e8f0" }}>
                ...
                {["Contract #", "Supplier Name", "Contract Officer", "Spend Period", "Total Spend", "Status"].map(h => (
                  <th
                    key={h}
                    style={{
                      padding: "10px 14px",
        ...
        {!loading && rows.length === 0 && (
          <div style={{ textAlign: "center", padding: "40px", color: "#6b7280", fontSize: 13 }}>
            No contracts found with status "{statusFilter.replace(/_/g, " ")}".
          </div>
        )}
      </div>
    </div>
