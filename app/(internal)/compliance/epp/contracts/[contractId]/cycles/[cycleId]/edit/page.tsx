...
export default function EditEppCycle() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (err) {
      ...
      router.push(`/compliance/epp/contracts/${contractId}`);
    }
    setSaving(false);
  }
  ...
  return (
    <div>
      <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 12 }}>
        ...
        {" / "}
        <Link href={`/compliance/epp/contracts/${contractId}`} style={{ color: "var(--usps-blue)" }}>
          {contractNum}
        </Link>
        {" / "}
        <Link href={`/compliance/epp/contracts/${contractId}/cycles/${cycleId}`} style={{ color: "var(--usps-blue)" }}>
          {cycle.name}
        </Link>
        {" / Edit"}
      </div>
      ...
      <div className="page-header">
        ...
        <Link href={`/compliance/epp/contracts/${contractId}`} className="btn btn-ghost">
          Cancel
        </Link>
      </div>
        ...
        <div className="card-body">
          <form onSubmit={handleSubmit} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div>
              ...
              <input className="form-input" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. FY2025 Q1" required />
            </div>
            <div>
              <label className="form-label">Fiscal Year</label>
              <input className="form-input" value={form.fiscal_year} onChange={e => setForm({ ...form, fiscal_year: e.target.value })} placeholder="e.g. FY2025" />
            </div>
            <div>
            ...
            <div style={{ gridColumn: "1 / -1" }}>
              ...
              <textarea className="form-textarea" value={form.goals} onChange={e => setForm({ ...form, goals: e.target.value })} rows={4} placeholder="EPP reporting goals for this cycle…" />
            </div>
            <div style={{ gridColumn: "1 / -1", display: "flex", justifyContent: "flex-end", gap: 8, paddingTop: 8, borderTop: "1px solid var(--border)" }}>
              <Link href={`/compliance/epp/contracts/${contractId}`} className="btn btn-ghost">Cancel</Link>
