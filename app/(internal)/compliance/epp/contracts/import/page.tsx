...
export default function EppContractImportPage() {
  ...
  function handleDownloadSample() {
    ...
    link.href = url;
    link.download = "epp_contracts_template.csv";
    link.click();
  ...
  return (
    <div style={{ padding: "24px", maxWidth: "1100px", margin: "0 auto" }}>
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title">Import EPP Contracts</h1>
          <p className="page-subtitle">
            Bulk-load EPP contracts from a CSV file. Download the template, fill in your data, then upload.
          </p>
        </div>
      </div>
            ...
            <table className="data-table">
              ...
              <tbody>
                {CSV_COLUMNS.map((col) => (
                  <tr key={col.header}>
                    <td>
                      <code style={{ background: "#f1f5f9", padding: "2px 6px", borderRadius: "4px", fontSize: "12px", fontFamily: "monospace", color: "var(--usps-blue)" }}>
                        {col.header}
                      </code>
                      {col.required && (
      ...
      <div className="card">
        ...
        <div className="card-body">
          ...
          <div style={{ marginBottom: "20px" }}>
            <label className="form-label" htmlFor="csv-upload">Select CSV File</label>
            <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
              <input
                ...
                style={{ fontSize: "13px", color: "var(--text)", border: "1.5px solid var(--border)", borderRadius: "6px", padding: "6px 10px", background: "white", cursor: "pointer", flex: 1, minWidth: "220px", maxWidth: "420px" }}
              />
              {parsedRows.length > 0 && (
                <button className="btn btn-ghost btn-sm" onClick={handleReset} type="button">Clear</button>
              )}
            </div>
                ...
                <table className="data-table" style={{ margin: 0 }}>
                  ...
                  <tbody>
                    {previewRows.map((row, idx) => (
                      <tr key={idx}>
                        {CSV_COLUMNS.map((col) => (
                          <td key={col.header} style={{ maxWidth: "180px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={row[col.header] ?? ""}>
                            {row[col.header] || <span style={{ color: "var(--border)" }}>—</span>}
                          </td>
                        ))}
                      </tr>
                    ))}
            ...
            <div style={{ marginBottom: "16px" }}>
              {result.successCount > 0 && (
                <div className="alert alert-success" style={{ marginBottom: "8px" }}>
                  {result.successCount} EPP contract{result.successCount !== 1 ? "s" : ""} imported successfully.
                </div>
              )}
              {result.errorCount > 0 && (
                <div className="alert alert-error" style={{ marginBottom: "8px", flexDirection: "column", alignItems: "flex-start", gap: "6px" }}>
                  <div>{result.errorCount} row{result.errorCount !== 1 ? "s" : ""} failed to import.</div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <button
              ...
              type="button"
            >
              {importing ? "Importing…" : `Import ${parsedRows.length > 0 ? `${parsedRows.length} Record${parsedRows.length !== 1 ? "s" : ""}` : "Records"}`}
            </button>
